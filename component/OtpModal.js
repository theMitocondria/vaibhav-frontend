...
const OtpModal = ({ email, isOpen, onClose }) => {
  ...
  const otphandleSubmit = async (e) => {
    ...
    const data = await fetch(
      "https://register-bpwp.onrender.com/auth/verify-user",
      {
        method: "POST",
        body: JSON.stringify({
          email,
  ...
  return (
    ...
    >
      <div className="bg-white py-4 w-full sm:w-1/2 max-w-[600px] rounded-lg shadow-lg">
        <div className="flex justify-around items-center align-middle">
          <h2 className="text-2xl text-black ml-8 mb-8 font-semibold mt-16 pt-16">
            Verify Otp
          </h2>
          <div className="relative sm:top-[-3rem] sm:right-[-6rem]">
            <button
        ...
        <div className="bg-white p-10   pb-16 leading-7   rounded-xl m-auto w-[80%] text-center">
          ...
          <input
            ...
            placeholder="Otp"
            className=" text-black mb-3 bord w-[80%] p-1 rounded-md outline-1"
          />
          <br />
          ...
          <button
            ...
            className=" bg-gray block mb-28 m-auto rounded-md text-center p-1 pl-3 pr-3  mt-8 text-lg  w-[90%] text-white hover:font-light"
          >
            Sign Up with Google
          </button>
        </div>
